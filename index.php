...
<?php endif;
  $numpics = 0;
    $scandir = scandir('../ircpics-archive/', $sort_order);
    shuffle( $scandir );
        ...
        if ( strpos($file, '.'.$extension ) > 0 ) :
          $numpics++;
            ...
            if ( $numpics > ( $starting_page -1 ) * $pics_per_page && $numpics <= $starting_page * $pics_per_page ) :
              ...
              ?>
                  <?php if ( 'masonry' === $layout ) : ?><a href="https://peikko.us/ircpics-archive/<?php echo $file; ?>"><?php endif; ?><img class="gallery-image" src="<?php if ( 'masonry' === $layout ) : ?>https://peikko.us/ircpics-archive/<?php echo rawurlencode($file); ?><?php else : ?><?php echo rawurlencode($file); ?><?php endif; ?>" alt="<?php echo $caption; ?>" /><?php if ( 'masonry' === $layout ) : ?></a><?php endif;
                break;
            endif;
        endif;
      endforeach;
...
<?php endif;
    ...
    <footer class="nav" style="display: none;">
        <?php
...
<p style="opacity: .5;"><small>Random IRC Pics by <b>rolle</b> @ quakenet ~ <b><?php echo count($files); ?></b> files ~ <a href="https://www.pulina.fi">www.pulina.fi</a></small></p>
